...
void terminalize(DecoderState& state, const StopPolicy& policy, PublishedOutput& emitted,
    ...
    if (state.in_reasoning) {
        ...
        // A held think-marker prefix ('<', '</thi', ...) is ordinary generated bytes that
        // ran out of stream before resolving. No further tokens can arrive, so publish it
        // as content; complete markers never reach this point because the non-reasoning
        // decode path already dropped them.
        if (!state.think_marker_pending.empty()) {
            feed_content(state, std::move(state.think_marker_pending), policy, emitted,
                         committed_tokens, nullptr);
            state.think_marker_pending.clear();
        }
