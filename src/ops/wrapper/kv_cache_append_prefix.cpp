...
void validate_cyclic_cache(const CyclicKVCacheLayerView& cache,
                           KVCacheAppendPrefixExecutionEnvelope envelope) {
    const bool power_of_two_window =
        cache.capacity >= 1024U && (cache.capacity & (cache.capacity - 1U)) == 0U;
    if (cache.num_kv_heads != kKVHeads || cache.head_dim != kHeadDim || !power_of_two_window ||
